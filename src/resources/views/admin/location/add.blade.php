        ...
        <div class="row">
            <div class="col-12">
                <div class="card border-0 box-shadow">
                    <div class="card-body">
                        <form action="{{URL::to('/admin/'. $locationurl.'/save-'.$section)}}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="row">
                                @if ($section == 1)
                                <div class="form-group col-md-6">
                                    <label class="form-label">{{trans('labels.pickup_location')}}<span class="text-danger"> * </span></label>
                                    <input type="text" class="form-control" name="pickup_location" value="{{old('pickup_location')}}" placeholder="{{trans('labels.pickup_location')}}" required>
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <input type="text" class="form-control" name="drop_location" value="{{old('drop_location')}}" placeholder="{{trans('labels.drop_location')}}" required>
                                    @error('drop_location')
                                    <span class="text-danger">{{ $message }}</span> 
                                 @enderror
                                </div>
